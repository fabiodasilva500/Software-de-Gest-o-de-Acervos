...
public class ServicoDeEmailAutomaticoView extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ServicoDeEmailAutomaticoView frame = new ServicoDeEmailAutomaticoView();
	 ...
	 * Create the frame.
	 */
	public ServicoDeEmailAutomaticoView() {
		...
		tfConteudo.setBounds(10, 454, 764, 58);
		contentPane.add(tfConteudo);

		JLabel lblConteudo = new JLabel("Conte\u00FAdo do Email");
		lblConteudo.setBounds(286, 429, 193, 14);
		contentPane.add(lblConteudo);

		btnEnviar.setBounds(157, 523, 89, 23);
		...
		tfAssunto.setBounds(231, 384, 421, 20);
		...
		btnVoltar.setBounds(526, 523, 89, 23);
		...
		ActionListener voltar=new ActionListener(){
			public void actionPerformed(ActionEvent e){
				ServicoDeEmailAutomaticoView.this.dispose();
			}
		};
		...
		ActionListener consultarDados=new ActionListener(){
			...
			public void actionPerformed(ActionEvent e){
				...
				if(!tfData.getText().isEmpty() && (data_requerida.before(data_atual))){
					...
					try {
						listaEmprestimosData= empDao.listaDeEmailsEmAtraso(emprestimo);
					...
					for (String emp: listaEmprestimosData){
						...
						if(emp!=""){
							tfAssunto.setText("Devolução de livros em atraso - Biblioteca Etec e Fatec Zona Leste.");
							tfConteudo.setText("Caro usuário, foi verificado em nosso acervo que você encontra-se com empréstimo(s) de livro(s) em atraso, por favor compareça a biblioteca para devolvê-lo. Atenciosamente, Biblioteca Etec/Fatec Zona Leste.");	
						}

						tfDestinatario.setText(a);
